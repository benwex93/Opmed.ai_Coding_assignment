...
3) No-overlap: AddNoOverlap(\{ $I_{i,a}$ \} for all $i$ per anesth $a$), and AddNoOverlap(\{ $J_{i,r}$ \} for all $i$ per room $r$).  
...
8) Shift linking: If $an_{i,a}=1$ then $start\_a \le start_i$ and $end\_a \ge end_i$. $shift_a = end_a - start_a$, $shift_a \le$ MAX\_SHIFT.  
