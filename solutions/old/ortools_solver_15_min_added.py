...
def read_surgeries(csv_path):
    ...
    df['end_min'] = ((df['end_dt'] - origin).dt.total_seconds() / 60).astype(int) + 15
...
def build_and_solve(df):
    ...
    # --------------------------------------------
    # 3️⃣ Cost model per anesthesiologist
    ...
    for i in range(n):
        ...
        sol.append({
            ...
            # Remove 15 minute surgery extension
            'end_min': int(df.loc[i, 'end_min']) - 15,
            'anesth': assigned_a,
            'room': assigned_r,
        })
    ...
    # Recalculate durations based on start and end times in the solution
    for i, row in enumerate(sol):
        row['dur_min'] = row['end_min'] - row['start_min']

    # Optional: if you want to update the original df as well
    for i, row in enumerate(sol):
        df.loc[i, 'dur_min'] = row['dur_min']

    # Recalculate total surgery minutes using updated durations
    total_surgery_minutes = sum(row['dur_min'] for row in sol)
    ...
    print(f"Anesthesiologists used: {used_count}")
    print(f"Total cost = {total_cost_hours:.2f} hours-equivalent")
    print(f"Utilization ratio = {utilization * 100:.1f}%")
